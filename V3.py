...
def scrape_fullh4rd(soup):
    # Precio
    price_tag = soup.find('span', {'class': 'bold'})
...
class ScraperActor(Actor):
    def receiveMessage(self, message, sender):
        ...
        if 'mercadolibre.com' in domain:
            price, availability, promotion = scrape_mercadolibre(soup)
        elif 'tiendamia.com' in domain:
            price, availability, promotion = scrape_tiendamia(soup)
        elif 'fullh4rd.com' in domain:
            price, availability, promotion = scrape_fullh4rd(soup)
        else:
            price, availability, promotion = 'Dominio no soportado', 'N/A', 'N/A'

        self.send(sender, (url, price, availability, promotion))
...
class CompareActor(Actor):
    def receiveMessage(self, message, sender):
        prices = message
        valid_prices = [(source, price, avail, promo) for source, price, avail, promo in prices if price != 'No encontrado' and price != 'Dominio no soportado']
